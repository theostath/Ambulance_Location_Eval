...
# line 41-42: define rt1
# line 65: define lamda
...
P = 10 # upper boundary of ambulances
NumOfStations = 3
...
for i in range(0,3):
    for j in range(0,12):
        if time[i][j]<=rt2:
            b[i][j]=1
################################################################
lamda = 0.42 # percentage of points of interest covered in time rt1 (this is set as a requirement)
...
y = p.var('y', 3, kind=int, bounds=(1,Pj)) # create 3 variables
...
for j in range(len(b[0])):
    R1=sum(y[i]*b[i][j] for i in range(3))>=1
...
for j in range(len(a[0])):
    R3=sum(y[i]*a[i][j] for i in range(3)) -x1[j] -x2[j] >=0
...
# constraint no 5
R5=sum(y[i] for i in range(3))<=P
#R6=sum(y[i] for i in range(3))>=P  # if i want to use all the ambulances, i also use this constraint
